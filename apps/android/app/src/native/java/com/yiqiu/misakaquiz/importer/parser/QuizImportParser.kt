package com.yiqiu.misakaquiz.importer.parser
...
import com.yiqiu.misakaquiz.importer.score.ImportStrategyScorer
...
object QuizImportParser {
    ...
    private fun repairQuestionForDisplay(question: Question): Question {
        return if (question.type == QuestionType.JUDGE && question.options.isEmpty()) {
            question.copy(
                options = listOf(
                    com.yiqiu.misakaquiz.importer.model.Option("A", "正确"),
                    com.yiqiu.misakaquiz.importer.model.Option("B", "错误")
                )
            )
        } else {
            question
