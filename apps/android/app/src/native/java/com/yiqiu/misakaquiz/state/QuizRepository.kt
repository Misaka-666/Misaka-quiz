package com.yiqiu.misakaquiz.state
...
object QuizRepository {
    ...
    private const val KEY_MISAKA_MODE_ENABLED = "misaka_mode_enabled"
    ...
    var darkThemeEnabled by mutableStateOf(false)
        private set
    var misakaModeEnabled by mutableStateOf(false)
        private set
    ...
    fun init(context: Context) {
        ...
        // Migrate from old "shiroha_quiz_native" SharedPreferences if new key is empty
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        if (prefs.getString(KEY_BANKS, null) == null) {
            val oldPrefs = context.getSharedPreferences("shiroha_quiz_native", Context.MODE_PRIVATE)
            val oldBanks = oldPrefs.getString(KEY_BANKS, null)
            if (oldBanks != null) {
                // Copy all old preferences to new storage
        ...
        misakaModeEnabled = prefs.getBoolean(KEY_MISAKA_MODE_ENABLED, false)
    ...
    fun mergeBanks(
        ...
        appContext = context.applicationContext
        val sourceBanks = banks.filter { it.id in bankIds }
        if (sourceBanks.size < 2) return null

        val cleanGroupName = normalizeBankGroupName(groupName)
        val cleanName = uniqueBankNameInGroup(
            newName.trim().ifBlank { "合并题库" },
            cleanGroupName
        )
        ...
        sourceBanks.forEach { bank ->
            bank.questions.forEach { q ->
                val normalized = normalizeQuestionTextForDedup(q.question)
                if (normalized !in seenTexts) {
                    seenTexts.add(normalized)
                    mergedQuestions.add(
                        q.copy(
                            id = "q_${UUID.randomUUID()}",
                            number = (mergedQuestions.size + 1).toString()
                        )
                    )
                }
            }
    ...
    private fun normalizeQuestionTextForDedup(text: String): String {
        return text
            .replace(Regex("[\\s\\u3000]"), "")
            .replace(Regex("[（）()\\[\\]【】{}。？?！!，,、：:；;.\\*]"), "")
    ...
    fun setMisakaModeEnabled(context: Context, enabled: Boolean) {
        appContext = context.applicationContext
        misakaModeEnabled = enabled
        persist()
        LauncherIconSwitcher.applyMisakaMode(context, enabled)
    }
    ...
    fun exportBanksBackupJson(bankIds: Set<String>): String {
        ...
        return buildBackupJson(
            kind = "misaka_quiz_selected_banks",
    ...
    private fun isWebBackupJson(root: JSONObject): Boolean {
        ...
        // Accept both new "misaka" and old "shiroha" identifiers for backward compatibility
        if (kind.startsWith("misaka_quiz_web") || kind.startsWith("shiroha_quiz_web") || root.has("crossPlatform")) return true
        if (kind.startsWith("misaka_quiz") || kind.startsWith("shiroha_quiz")) return false
        val app = root.optString("app")
        return app.equals("Misaka Quiz", ignoreCase = true) ||
            app.equals("Shiroha Quiz", ignoreCase = true) ||
    ...
    private fun persist() {
        ...
        prefs.edit()
            ...
            .putBoolean(KEY_MISAKA_MODE_ENABLED, misakaModeEnabled)
