...
class WebShellActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        webView = WebView(this).apply {
            ...
            addJavascriptInterface(MisakaBridge(), "MisakaAndroid")
    ...
    inner class MisakaBridge {
        @JavascriptInterface
        fun saveJsonFile(fileName: String, content: String): Boolean {
            val safeName = sanitizeFileName(fileName.ifBlank { "misaka-quiz-backup.json" })
            return runCatching {
    ...
    private fun sanitizeFileName(name: String): String {
        return name.replace(Regex("[\\\\/:*?\"<>|]"), "_")
            .replace(Regex("\\s+"), "_")
            .take(96)
            .ifBlank { "misaka-quiz-backup.json" }
    }
