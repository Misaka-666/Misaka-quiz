...
init();
function init(){upgradeState();ensureDefaultBank();bindNav();bindEvents();bindDashboardStudyPanel();setupQuestionImageViewerV302();renderBankSelect();renderAll();setupEnhancedDataToolsV23();}
...
$('#load-sample-btn').onclick=loadSample;$('#import-file').onchange=readImportFile;$('#parse-import-btn').onclick=parseImport;$('#confirm-import-btn').onclick=confirmImport;const dualConfirmBtn=$('#dual-confirm-import-btn');if(dualConfirmBtn)dualConfirmBtn.onclick=confirmImport;const importTextAreaV49=$('#import-text');if(importTextAreaV49)importTextAreaV49.addEventListener('input',()=>{if(importTextAreaV49.dataset.tableImportV49==='1'){tableImportResultV49=null;delete importTextAreaV49.dataset.tableImportV49;}});$('#clear-import-btn').onclick=clearImportWorkspace;
$('#dual-question-file').onchange=e=>readDualFile(e,'question');$('#dual-answer-file').onchange=e=>readDualFile(e,'answer');$('#parse-dual-import-btn').onclick=parseDualImport;$('#clear-dual-import-btn').onclick=clearDualImportWorkspace;$('#dual-load-sample-btn').onclick=loadDualSample;$('#revalidate-import-btn').onclick=()=>renderImportPreview(importCache);
$('#edit-close-btn').onclick=closeEditModal;$('#edit-save-btn').onclick=saveEditQuestion;$('#edit-delete-btn').onclick=deleteEditQuestion;const pf=$('#import-preview-filter');if(pf)pf.onchange=e=>{importPreviewFilter=e.target.value;renderImportPreview(importCache)};const bid=$('#batch-delete-import-btn');if(bid)bid.onclick=batchDeleteImportSelected;const cis=$('#clear-import-selection-btn');if(cis)cis.onclick=()=>{importSelected.clear();renderImportPreview(importCache)};const ipfBtn=$('#import-preview-focus-btn');if(ipfBtn)ipfBtn.onclick=toggleImportPreviewFocus;document.addEventListener('keydown',e=>{if(e.key==='Escape'&&document.body.classList.contains('import-preview-focus'))toggleImportPreviewFocus(false)});
...
function formatDashboardTodayCount(){const today=new Date().toISOString().slice(0,10);return (state.records||[]).filter(r=>String(r.date||'').slice(0,10)===today).reduce((sum,r)=>sum+Number(r.answered||0),0)}
function dashboardAdviceV25(total,todayCount,wrongCount){const target=20;if(!total)return '先加载题库，再开始练习';if(wrongCount>0&&todayCount>=target)return `今日目标已完成，还有 ${wrongCount} 道错题可复习`;if(wrongCount>0)return `有 ${wrongCount} 道待复习，建议先复习错题`;if(todayCount<=0)return '今天还没开始，建议先做 20 题';if(todayCount<target)return `已完成 ${todayCount} 题，再做 ${target-todayCount} 题达成今日目标`;return '今日目标已完成，可以复习错题或继续练习'}
function renderStats(){const b=activeBank();const total=(b.questions||[]).length;const wrongCount=(state.wrongBook[b.id]||[]).length;const recordCount=(state.records||[]).length;const todayCount=formatDashboardTodayCount();$('#stat-total').textContent=total;$('#stat-wrong').textContent=wrongCount;$('#stat-records').textContent=recordCount;const bankTitle=$('#dashboard-bank-title');if(bankTitle)bankTitle.textContent=b.name||'未命名题库';const bankNameEl=$('#dashboard-bank-name');if(bankNameEl)bankNameEl.textContent=`共 ${total} 题 · ${wrongCount?wrongCount+' 道待复习':'暂无待复习'}`;const tip=$('#dashboard-study-tip');if(tip)tip.textContent=dashboardAdviceV25(total,todayCount,wrongCount);const dashTotal=$('#dashboard-stat-total');if(dashTotal)dashTotal.textContent=`${total}题`;const dashToday=$('#dashboard-stat-today');if(dashToday)dashToday.textContent=`${todayCount}题`;const dashReview=$('#dashboard-stat-review');if(dashReview)dashReview.textContent=`${wrongCount}道`;}
function bindDashboardStudyPanel(){const btn=$('#dashboard-start-practice-btn');if(btn)btn.onclick=()=>{document.querySelector('.nav[data-view="practice"]')?.click();try{window.scrollTo({top:0,behavior:'smooth'})}catch(_){window.scrollTo(0,0)}}}
...
function renderBankList(){
  ...
  box.innerHTML=banks.map(b=>{
    ...
    return `<article class="bank-item bank-item-compact-v28 ${active?'active-bank':''} ${checked?'selected-bank-v28':''}">
      <label class="bank-bulk-check-v23 bank-card-check-v28" title="选择后可批量导出"><input type="checkbox" data-bank-bulk-v23="${esc(b.id)}" ${checked?'checked':''}></label>
...
function renderBankToolbarV28(){
  ...
  sel.innerHTML=state.banks.map(b=>`<option value="${esc(b.id)}">${esc(b.name)}（${b.questions.length}题）</option>`).join('')||'<option value="">暂无题库</option>';
...
function tableRowToQuestionV49(row,map,idx,sheetName=''){
  ...
  return {id:'tbl_'+Date.now()+'_'+idx,type,number:num||idx+1,question,options:options.filter(o=>o.text),answer:answer.length?answer:correctFromOptions,analysis,category,group:category,score:tableCellV49(row,map.score)};
}
  ...
  if(relEntry){
    try{
      ...
      for(const [rid,target] of Object.entries(rels)){
        ...
        if(/^image\/(?:png|jpe?g|gif|webp|bmp|svg\+xml)$/i.test(mime)){
...
function mergeAnswerAnalysisEntries(questions,entries){
  ...
  for(const e of (entries||[])){
    ...
    qs[idx].analysis=formatAnswerAnalysisForReview(qs[idx].answer,e.analysis||qs[idx].analysis||'');
...
function makeRecruitmentQuestion(seg,ctx,idx,restore){
  ...
  const q={id:'imp_'+Date.now()+'_'+idx,type,number:num,question,options,answer:ans,analysis:ctx.analysis||'',group,category:group};
...
function parseBlock(block,idx){
  ...
  return {id:'imp_'+Date.now()+'_'+idx,type,number,question,options,answer,analysis:formatAnswerAnalysisForReview(answer,analysis.trim()),group};
}
...
function clearImportWorkspace(){
  if(bankEditContext){exitBankEditor(false);return}
  const textEl=$('#import-text');if(textEl){textEl.value='';delete textEl.dataset.tableImportV49;}tableImportResultV49=null;importCache=[];importSelected.clear();importWarnings=[];importReport='';importDiagnostics=null;renderImportPreview([]);
}
function clearDualImportWorkspace(){
  if(bankEditContext){exitBankEditor(false);return}
  $('#dual-question-text').value='';$('#dual-answer-text').value='';tableImportResultV49=null;importCache=[];importSelected.clear();importWarnings=[];importReport='';importDiagnostics=null;renderImportPreview([]);
}
function activateViewV13(id){
  const view=$('#'+id);if(!view)return;
  $$('.nav').forEach(btn=>btn.classList.toggle('active',btn.dataset.view===id));
  $$('.view').forEach(v=>v.classList.toggle('active',v===view));
  const title=$('#page-title');const nav=$(`.nav[data-view="${id}"]`);if(title&&nav)title.textContent=nav.textContent;
  updateShellLayoutByView(id);resetViewScrollV282();
}
function cloneBankQuestionsForEditV13(bank){
  return (bank.questions||[]).map((q,i)=>normalizeQuestion(JSON.parse(JSON.stringify(q)),i));
}
function openBankEditor(id){
  const bank=state.banks.find(b=>b.id===id);if(!bank){toast('没有找到该题库。','warn');return}
  bankEditContext={bankId:bank.id,bankName:bank.name||'未命名题库',startedAt:now()};
  state.activeBankId=bank.id;saveSilent();
  importCache=cloneBankQuestionsForEditV13(bank);
  importWarnings=[];importDiagnostics=null;importSelected.clear();importPreviewFilter='all';
  importReport=`题库编辑模式：正在重新核对“${bankEditContext.bankName}”的每一道题。可查看题型、题干、选项数、答案和异常状态，点击单题“编辑”进行二次修改，完成后点“保存回题库”。`;
  activateViewV13('import');renderAll();renderImportPreview(importCache);
  setTimeout(()=>{$('.import-preview-card')?.scrollIntoView({block:'start'});},40);
}
function ensureBankEditToolbarV13(){
  let bar=$('#bank-edit-toolbar-v13');
  if(!bar){
    const summary=$('#import-summary');
    if(!summary)return null;
    summary.insertAdjacentHTML('beforebegin','<div id="bank-edit-toolbar-v13" class="bank-edit-toolbar-v13 hidden"></div>');
    bar=$('#bank-edit-toolbar-v13');
  }
  return bar;
}
function syncBankEditUiV13(){
  const editing=!!bankEditContext;
  document.body.classList.toggle('bank-edit-mode-v13',editing);
  const h=$('.import-preview-card .section-head h2');if(h)h.textContent=editing?'题库识别详情编辑':'识别结果预览';
  const p=$('.import-preview-card .section-head .muted');if(p)p.textContent=editing?'重新查看每道题的识别状态，二次修改后保存回原题库。':'核对题型、答案和异常提示后再导入。';
  const confirmBtn=$('#confirm-import-btn');if(confirmBtn)confirmBtn.textContent=editing?'保存回题库':'确认导入';
  const dualConfirm=$('#dual-confirm-import-btn');if(dualConfirm)dualConfirm.textContent=editing?'保存回题库':'确认导入';
  const bar=ensureBankEditToolbarV13();
  if(!bar)return;
  if(!editing){bar.classList.add('hidden');bar.innerHTML='';return}
  const bank=state.banks.find(b=>b.id===bankEditContext.bankId);
  const count=(importCache||[]).length;
  bar.classList.remove('hidden');
  bar.innerHTML=`<div><b>正在编辑：${esc(bank?.name||bankEditContext.bankName)}</b><span>共 ${count} 题，修改只会在点击“保存回题库”后写入本地题库。</span></div><div class="row-actions"><button id="bank-edit-save-v13" class="primary" type="button">保存回题库</button><button id="bank-edit-exit-v13" class="ghost" type="button">退出编辑</button></div>`;
  $('#bank-edit-save-v13').onclick=saveBankEditor;
  $('#bank-edit-exit-v13').onclick=()=>exitBankEditor(false);
}
function saveBankEditor(){
  if(!bankEditContext){confirmImport();return}
  const bank=state.banks.find(b=>b.id===bankEditContext.bankId);
  if(!bank){toast('原题库不存在，无法保存。','danger');return}
  const warnings=collectImportWarnings(importCache);
  if(warnings.length&&!confirm(`当前还有 ${warnings.length} 条异常/风险提示，仍然保存回题库吗？`))return;
  const stamp=Date.now();
  bank.questions=importCache.map((q,i)=>cleanImportedQuestion({...q,id:q.id||('q_'+stamp+'_'+i),number:i+1}));
  bank.updatedAt=now();state.activeBankId=bank.id;saveSilent();renderAll();renderImportPreview(importCache);
  toast(`已保存到题库“${bank.name}”，共 ${bank.questions.length} 题。`,'ok','保存成功');
}
function exitBankEditor(saved){
  if(bankEditContext&&!saved){
    const ok=confirm('退出题库编辑？未保存的修改不会写回题库。');
    if(!ok)return;
  }
  bankEditContext=null;importCache=[];importSelected.clear();importWarnings=[];importReport='';importDiagnostics=null;importPreviewFilter='priority';
  renderImportPreview([]);syncBankEditUiV13();activateViewV13('banks');renderAll();
}

function toggleImportPreviewFocus(force){
  const on=typeof force==='boolean'?force:!document.body.classList.contains('import-preview-focus');
  document.body.classList.toggle('import-preview-focus',on);
  const btn=$('#import-preview-focus-btn');
  if(btn){btn.textContent=on?'退出沉浸':'沉浸核对';btn.setAttribute('aria-pressed',on?'true':'false')}
  if(on){
    if(document.documentElement.requestFullscreen){document.documentElement.requestFullscreen().catch(()=>{});}
    setTimeout(()=>{$('.import-preview-card')?.scrollIntoView({block:'start'});},20);
...
function renderImportPreview(arr){
  ...
  const previewRowsHtml=shown.map(({q,i,status})=>{
    ...
    return `<tr class="${rowCls}"><td class="select-cell"><input type="checkbox" class="import-row-check" data-select-import="${i}" ${checked}></td><td class="seq-cell"><b>${i+1}</b>${seqInfo?`<small>${esc(seqInfo)}</small>`:''}</td><td>${label(q.type)}</td><td>${esc(short(q.question,72))}</td><td>${optionCount}</td><td>${esc(answerText)}</td><td class="${cls}">${esc(status)}</td><td><div class="row-actions"><button class="ghost mini-btn" data-edit-import="${i}">编辑</button><button class="ghost danger mini-btn" data-delete-import="${i}">删除</button></div></td></tr>`
  }).join('');
  $('#import-preview tbody').innerHTML=previewRowsHtml;
  const mobileList=$('#import-preview-mobile');
  if(mobileList){
    mobileList.innerHTML=shown.map(({q,i,status})=>{
      const severity=importStatusSeverity(status);
      const statusCls=severity==='ok'?'status-ok':severity==='error'?'status-error':'status-warn';
      const cardCls=severity==='ok'?'':severity==='error'?'error-card':'issue-card';
      const seqInfo=[q.volume,q.group,q.number?`原${q.number}`:''].filter(Boolean).join(' · ');
      const checked=importSelected.has(i)?'checked':'';
      const answerText=Array.isArray(q.answer)?q.answer.join(''):String(q.answer||'');
      const optionCount=Array.isArray(q.options)?q.options.length:0;
      return `<article class="import-preview-item ${cardCls}">
        <label class="import-preview-check"><input type="checkbox" class="import-row-check" data-select-import="${i}" ${checked}><span>选择</span></label>
        <div class="import-preview-item-main">
          <div class="import-preview-item-head"><b>第 ${i+1} 题 · ${label(q.type)}</b><span class="${statusCls}">${esc(status)}</span></div>
          ${seqInfo?`<div class="import-preview-meta">${esc(seqInfo)}</div>`:''}
          <div class="import-preview-question">${esc(short(q.question,120))}</div>
          <div class="import-preview-meta-grid"><span>选项数：${optionCount}</span><span>答案：${esc(answerText||'未识别')}</span></div>
          <div class="import-card-actions"><button class="ghost mini-btn" data-edit-import="${i}">编辑</button><button class="ghost danger mini-btn" data-delete-import="${i}">删除</button></div>
        </div>
      </article>`
    }).join('');
  }
  ...
  $('#confirm-import-btn').disabled=!arr.length;const dualConfirm=$('#dual-confirm-import-btn');if(dualConfirm)dualConfirm.disabled=!arr.length;syncBankEditUiV13();
  ...
  $$('[data-delete-import]').forEach(btn=>btn.onclick=()=>{const i=Number(btn.dataset.deleteImport);if(confirm(bankEditContext?'从当前题库编辑副本中删除这道题？保存回题库后生效。':'删除这道识别结果？')){importCache.splice(i,1);importSelected=new Set([...importSelected].map(x=>x>i?x-1:x).filter(x=>x!==i));renderImportPreview(importCache)}});
}
function batchDeleteImportSelected(){
  ...
  if(!confirm(bankEditContext?`从当前题库编辑副本中删除选中的 ${ids.length} 道题？保存回题库后生效。`:`确定批量删除选中的 ${ids.length} 道识别结果？`))return;
...
function openEditQuestion(i){
  ...
  $('#edit-status').textContent=bankEditContext?'可修改后保存到编辑副本，最后点“保存回题库”。':'可修改后保存。';$('#edit-status').className='notice';
...
function saveEditQuestion(){
  ...
  importCache[i]=q;
  const status=validateQuestion(q);
  $('#edit-status').textContent=(bankEditContext?'已保存到编辑副本。当前状态：':'已保存。当前状态：')+status;
...
function confirmImport(){
  if(bankEditContext){saveBankEditor();return}
...
function startWrongPractice(){
  $$('.nav').forEach(b=>b.classList.remove('active'));document.querySelector('[data-view="practice"]').classList.add('active');$$('.view').forEach(v=>v.classList.remove('active'));$('#practice').classList.add('active');const pageTitle=$('#page-title');if(pageTitle)pageTitle.textContent='刷题练习';
...
function questionHtml(q,examMode,idx=0){
  ...
  const optionTextLen=opts.reduce((m,o)=>Math.max(m,String(o.text||'').trim().length),0);
  const simpleOptions=opts.length>0&&opts.length<=5&&optionTextLen<=8&&!hasQuestionImageContent(String(q.question||''));
  const optionsClass=simpleOptions?'options simple-options':'options';
  return meta+`<div class="${optionsClass}">${opts.map(o=>`<label class="option" data-key="${esc(o.key)}"><input type="${q.type==='multiple'?'checkbox':'radio'}" name="q_${esc(q.id)}" value="${esc(o.key)}"><span class="option-key">${esc(o.key)}.</span><span>${esc(o.text)}</span></label>`).join('')}</div>`;
}
...
function finishPractice(exited=false){
  ...
  if(total>0)state.records.unshift(rec);
  saveSilent();
  exitPracticeFocus();
  $('#practice-progress').textContent='0 / 0';
  $('#practice-card').innerHTML=`<div class="empty compact-end-v5">${total>0?'本轮练习已结束，作答记录已保存到记录。':'本轮练习已退出，未产生作答记录。'}</div>`;
  practice={items:[],idx:0,answered:0,correct:0,wrong:0,start:0,details:[],answerState:{}};
  showNotice(exited?'练习已退出':'练习完成',total>0?`已答 ${total} 道，正确率 ${rec.accuracy}%。可在记录查看明细。`:'本轮未作答。','ok');
  renderAll();
}
...
function ensureBankManageExportPanelV23(){
  ...
  bankList.insertAdjacentHTML('beforebegin',`<div id="bank-bulk-export-panel-v23" class="data-tool-card-v23 bank-bulk-panel-v23 bank-manage-toolbar-v28">
    <div class="section-head compact-head-v23"><div><h3>当前题库与批量导出</h3><p class="muted">当前题库改为在上方统一选择；下方卡片只展示题库信息。需要批量导出时，勾选题库后使用这里的导出按钮。</p></div></div>
    <div class="bank-current-bar-v28">
      <label>当前题库<select id="bank-current-select-v28"></select></label>
      <span class="muted">切换后会同步顶部当前题库，并刷新预览、练习和错题范围。</span>
    </div>
...
function normalizeBackupPayloadV23(data,fileName){
  ...
  let banks=[];let wrongBook={};let records=[];let settings={};let activeBankId='';
...
function mergeBackupBanksV23(normalized){
  ...
  normalized.banks.forEach((bank,idx)=>{
    ...
    if(existingNames.has(name)){let n=1;const base=name+'_导入';while(existingNames.has(base+n))n++;name=base+n}
    let id=bank.id;if(existingIds.has(id))id='bank_import_'+Date.now()+'_'+idx;
    existingIds.add(id);existingNames.add(name);
    const next={...bank,id,name,createdAt:bank.createdAt||now(),updatedAt:now(),questions:(bank.questions||[]).map((q,i)=>({...normalizeQuestion(q,i),id:q.id||('q_'+Date.now()+'_'+idx+'_'+i),number:q.number||i+1}))};
...
*/
function init(){upgradeState();ensureDefaultBank();bindNav();bindEvents();bindDashboardStudyPanel();bindV25ToV28Events();ensureV25ToV28Panels();renderBankSelect();renderAll();setupEnhancedDataToolsV23();updateShellLayoutByView();}
function defaultBank(){
  ...
  return {id:'default-c1',name:qs.length?(qb.meta?.title||'默认题库'):'内置题库（待加载）',createdAt:now(),updatedAt:now(),questions:qs.map(normalizeQuestion),builtInLazy:!qs.length};
}
function upgradeState(){
  state.schemaVersion=CURRENT_SCHEMA_VERSION;
  state.banks=Array.isArray(state.banks)?state.banks:[];
...
function renderAll(){renderStats();renderBankSelect();renderMergeSelect();renderBankList();renderBankPreview();renderWrongBook();renderRecords();renderFavoritePanelV27();renderBankInputs();renderBuiltInPanelV252();if(typeof renderExportBankSelectorV23==='function')renderExportBankSelectorV23();}
...
function ensureBuiltInPanelV252(){
  // WebView App 默认已有内置 C1 题库，不再插入“内置题库按需加载”提示卡，避免题库管理页多出无意义卡片。
...
function renderPracticeNavV26(){
  ...
  return `<div class="practice-nav-v26"><b>答题卡</b><div class="practice-nav-grid-v26">${buttons}</div></div>`;
}
function submitPractice(q,reveal){
  ...
  markOptions('#practice-card',q,chosen);showAnsweredStateV26(q,getPracticeAnswerStateV26(q.id));saveSilent();renderStats();
}
...
function mergeBackupBanksV23(normalized){
  const existingNames=new Set(state.banks.map(b=>b.name));const existingIds=new Set(state.banks.map(b=>b.id));
  normalized.banks.forEach((bank,idx)=>{const oldId=bank.id;let name=bank.name||('导入题库_'+(idx+1));if(existingNames.has(name)){let n=1;const base=name+'_导入';while(existingNames.has(base+n))n++;name=base+n}let id=bank.id;if(existingIds.has(id))id='bank_import_'+Date.now()+'_'+idx;existingIds.add(id);existingNames.add(name);const next={...bank,id,name,createdAt:bank.createdAt||now(),updatedAt:now(),questions:(bank.questions||[]).map((q,i)=>({...normalizeQuestion(q,i),id:q.id||('q_'+Date.now()+'_'+idx+'_'+i),number:q.number||i+1}))};state.banks.push(next);if(normalized.wrongBook&&normalized.wrongBook[oldId])state.wrongBook[id]=normalized.wrongBook[oldId];if(normalized.favorites&&normalized.favorites[oldId])state.favorites[id]=normalized.favorites[oldId];});
  state.activeBankId=state.banks[state.banks.length-normalized.banks.length]?.id||state.activeBankId;
}
...
function examAnsweredCountV7(){
  if(!Array.isArray(exam.items))return 0;
  return exam.items.reduce((n,q)=>n+(((exam.answers&&exam.answers[q.id])||[]).length?1:0),0);
}
function examAnswerSheetButtonV8(){
  const answered=examAnsweredCountV7();
  const total=Array.isArray(exam.items)?exam.items.length:0;
  return `<button class="exam-sheet-toggle-v7" id="exam-sheet-toggle-v7" type="button" aria-expanded="false"><span>答题卡</span><em id="exam-sheet-count-v7">${answered}/${total}</em></button>`;
}
function examAnswerSheetPanelV8(){
  const answered=examAnsweredCountV7();
  const total=Array.isArray(exam.items)?exam.items.length:0;
  const nums=(exam.items||[]).map((q,i)=>`<button type="button" class="${((exam.answers&&exam.answers[q.id])||[]).length?'answered':''}" data-exam-jump="${i}">${i+1}</button>`).join('');
  return `<div class="exam-sheet-panel-v7" id="exam-sheet-panel-v7" hidden><div class="exam-sheet-summary-v7"><span>已答 <b id="exam-sheet-answered-v7">${answered}</b> / ${total}</span><span class="dot answered"></span><span>已答</span><span class="dot"></span><span>未答</span></div><div class="exam-sheet-grid-v7">${nums}</div></div>`;
}
function examAnswerSheetBlockV7(){
  return `${examAnswerSheetButtonV8()}${examAnswerSheetPanelV8()}`;
}
function updateExamSheetStateV7(){
  const total=Array.isArray(exam.items)?exam.items.length:0;
  const answered=examAnsweredCountV7();
  const count=$('#exam-sheet-count-v7');if(count)count.textContent=`${answered}/${total}`;
  const answeredEl=$('#exam-sheet-answered-v7');if(answeredEl)answeredEl.textContent=String(answered);
  $$('#exam-sheet-panel-v7 [data-exam-jump]').forEach(btn=>{
    const i=Number(btn.dataset.examJump);
    const q=exam.items&&exam.items[i];
    btn.classList.toggle('answered',!!(q&&exam.answers&&exam.answers[q.id]&&exam.answers[q.id].length));
  });
}
function bindExamSheetV7(){
  const toggle=$('#exam-sheet-toggle-v7');
  const panel=$('#exam-sheet-panel-v7');
  if(toggle&&panel){
    toggle.onclick=()=>{
  ...
  $$('#exam-sheet-panel-v7 [data-exam-jump]').forEach(btn=>{
    btn.onclick=()=>{
      const i=Number(btn.dataset.examJump);
      const target=$(`#exam-card .exam-q[data-exam-index="${i}"]`);
...
function renderExamPaper(){
  ...
  let html=`<div class="exam-focus-head"><div class="exam-head-title"><b>${esc(exam.name||'模拟考试')}</b><span id="exam-focus-timer">${esc(timerText)}</span></div><div class="exam-head-sheet-v8">${examAnswerSheetButtonV8()}</div><div class="exam-head-actions"><button class="ghost mini-btn focus-mini-btn" id="exam-exit-focus" type="button">退出考试</button><button class="danger focus-mini-btn" id="exam-submit-focus" type="button">交卷评分</button></div></div>${examAnswerSheetPanelV8()}<div class="notice warn">考试中：多选题需完全一致才得分；填空/简答按参考答案规范化匹配评分，简答题建议交卷后人工核对。</div>`;
  exam.items.forEach((q,i)=>{html+=`<div class="exam-q" data-qid="${esc(q.id)}" data-exam-index="${i}">${questionHtml(q,true,i+1)}</div>`});
  ...
  bindExamSheetV7();
  $$('#exam-card .option').forEach(opt=>{opt.onclick=()=>setTimeout(()=>{const box=opt.closest('.exam-q');const id=box.dataset.qid;exam.answers[id]=selectedKeys(`[data-qid="${CSS.escape(id)}"]`);box.querySelectorAll('.option').forEach(o=>o.classList.toggle('selected',o.querySelector('input').checked));updateExamSheetStateV7()},0)});
  $$('#exam-card .text-answer').forEach(el=>{el.oninput=()=>{const box=el.closest('.exam-q');if(box)exam.answers[box.dataset.qid]=el.value.trim()?[el.value.trim()]:[];updateExamSheetStateV7()}});
  updateExamSheetStateV7();
}
function switchToRecordsViewV5(mode){
  const target=$('#records');
  if(!target)return;
  $$('.view').forEach(v=>v.classList.toggle('active',v===target));
  $$('.nav').forEach(btn=>btn.classList.toggle('active',btn.dataset.view==='records'));
  const title=$('#page-title');if(title)title.textContent='记录';
  const filter=$('#record-mode-filter');if(filter&&mode)filter.value=mode;
  updateShellLayoutByView('records');
  renderRecords();
  resetViewScrollV282();
}
function submitExam(auto){
  ...
  state.records.unshift(rec);saveSilent();
  $('#submit-exam-btn').disabled=true;$('#exam-timer').textContent=auto?'已自动交卷':'已交卷';
  $('#exam-card').innerHTML='<div class="empty compact-end-v5">考试已交卷，结果已保存到记录。</div>';
  exam={items:[],answers:{},start:0,timer:null,deadline:0,submitted:true};
  exitExamFocus();
  renderAll();
  switchToRecordsViewV5('考试');
  showNotice(auto?'自动交卷完成':'考试已交卷',`得分 ${rec.score}/${rec.totalScore}，正确率 ${rec.accuracy}%。明细已保存到记录。`,rec.passed?'ok':'warn');
}
...
function setupQuestionImageViewerV302(){
  ...
  document.body.dataset.questionImageViewerV302='1';

  const ensureViewer=()=>{
    let viewer=document.getElementById('question-image-viewer-v302');
    if(viewer)return viewer;
    viewer=document.createElement('div');
    viewer.id='question-image-viewer-v302';
    viewer.className='question-image-viewer-v302';
    viewer.innerHTML='<button class="question-image-viewer-close-v302" type="button" aria-label="关闭图片预览">×</button><div class="question-image-viewer-stage-v302"><img alt="题目图片预览"></div><div class="question-image-viewer-tip-v302">点击空白处关闭，可双指缩放查看细节</div>';
    document.body.appendChild(viewer);
    viewer.addEventListener('click',e=>{
      if(e.target===viewer||e.target.classList.contains('question-image-viewer-close-v302'))closeViewer();
    });
    return viewer;
  };
  ...
  const openViewer=(img)=>{
    ...
    preview.alt=img.alt||'题目图片预览';
    viewer.classList.add('active');
    document.body.classList.add('image-preview-open-v302');
  };
  const closeViewer=()=>{
    const viewer=document.getElementById('question-image-viewer-v302');
    if(!viewer)return;
    viewer.classList.remove('active');
    document.body.classList.remove('image-preview-open-v302');
    setTimeout(()=>{const img=viewer.querySelector('img');if(img&&!viewer.classList.contains('active'))img.removeAttribute('src')},180);
  };

  document.addEventListener('click',e=>{
    const img=e.target&&e.target.closest?e.target.closest('.question-image'):null;
    if(!img)return;
    e.preventDefault();
    openViewer(img);
  });
  ...
  document.addEventListener('load',e=>{
    const img=e.target;
    if(!img||!img.classList||!img.classList.contains('question-image'))return;
    const fig=img.closest('.question-media');
    if(!fig)return;
    const w=img.naturalWidth||0,h=img.naturalHeight||0;
    if(!w||!h)return;
    fig.classList.toggle('is-wide-image',w/h>=1.45&&w>=760);
    fig.classList.toggle('is-tall-image',h/w>=1.35&&h>=760);
    fig.dataset.imageRatio=(w/h).toFixed(2);
    img.title='点击查看大图';
  },true);
}
...
function epochMillisV8215(value){
...
function scoreValueV8215(value){
  const n=Number(value);
  return Number.isFinite(n)&&n>0?n:undefined;
}
function arrayFromV8215(value){return Array.isArray(value)?value.filter(x=>x!=null).map(x=>String(x)):[]}
function nativeOptionForExportV8215(option,index){
  if(typeof option==='string')return{key:String.fromCharCode(65+index),text:option};
  return{key:normalizeOptionKey(option?.key||option?.label||option?.value||String.fromCharCode(65+index)),text:String(option?.text||option?.content||option?.label||'')};
}
function nativeQuestionForExportV8215(question,index=0){
  const q=normalizeQuestion(question||{},index);
  const out={
    id:String(q.id||question?.id||('q_'+Date.now()+'_'+index)),
    number:String(q.number||question?.number||index+1),
    type:nativeTypeForExportV8215(q.type||question?.type),
    question:String(q.question||''),
    analysis:String(q.analysis||''),
    category:String(q.category||''),
    options:(q.options||[]).map(nativeOptionForExportV8215).filter(o=>o.text),
    answer:arrayFromV8215(q.answer),
    images:Array.isArray(q.images)?q.images:[]
  };
  const score=scoreValueV8215(q.score??question?.score);
  if(score!==undefined)out.score=score;
  return out;
}
function nativeBankForExportV8215(bank,index=0){
  const questions=Array.isArray(bank?.questions)?bank.questions:[];
  return{
    id:String(bank?.id||('bank_'+Date.now()+'_'+index)),
    name:String(bank?.name||bank?.title||('题库_'+(index+1))),
    questions:questions.map((q,i)=>nativeQuestionForExportV8215(q,i)).filter(q=>q.question)
  };
}
function findQuestionForExportV8215(bankId,questionId){
  const bank=(state.banks||[]).find(b=>b.id===bankId)||activeBank();
  return bank?.questions?.find(q=>q.id===questionId)||null;
}
function nativeWrongBookForExportV8215(bankIds,includeAll=false){
  const entries=[];
  const ids=bankIds||new Set();
  Object.entries(state.wrongBook||{}).forEach(([bankId,list])=>{
    if(!includeAll&&!ids.has(bankId))return;
    const bank=(state.banks||[]).find(b=>b.id===bankId);
    (Array.isArray(list)?list:[]).forEach((entry,idx)=>{
      const q=findQuestionForExportV8215(bankId,entry?.id||entry?.questionId);
      if(!q)return;
      const timestamp=epochMillisV8215(entry.lastWrongAt||entry.timestamp||entry.time||now());
      const item={
        bankId:String(bankId),
        bankName:String(bank?.name||''),
        source:'Web',
        timestamp,
        lastAnswer:arrayFromV8215(entry.lastAnswer||entry.chosen||entry.answer),
        wrongCount:Number(entry.wrongCount||1)||1,
        rightCount:Number(entry.rightCount||0)||0,
        lastWrongAt:epochMillisV8215(entry.lastWrongAt||timestamp),
        status:String(entry.status||'未掌握'),
        question:nativeQuestionForExportV8215(q,idx)
      };
      if(entry.lastCorrectAt)item.lastCorrectAt=epochMillisV8215(entry.lastCorrectAt);
      entries.push(item);
    });
  });
  return entries;
}
function nativeStudyRecordsForExportV8215(includeAll=false){
  if(!includeAll)return[];
  return (state.records||[]).map((record,idx)=>{
    const bank=(state.banks||[]).find(b=>b.id===record.bankId)||activeBank();
    const timestamp=epochMillisV8215(record.date||record.timestamp||now());
    const duration=Number(record.duration||record.durationSeconds||0)||0;
    return{
      id:String(record.id||('rec_'+Date.now()+'_'+idx)),
      bankId:record.bankId?String(record.bankId):null,
      bankName:String(record.bankName||bank?.name||''),
      source:String(record.mode||record.source||'Web'),
      title:String(record.name||record.title||record.mode||'学习记录'),
      total:Number(record.total||0)||0,
      correct:Number(record.correct||0)||0,
      timestamp,
      durationSeconds:duration,
      autoSubmitted:!!record.autoSubmitted,
      startedAt:record.startedAt?epochMillisV8215(record.startedAt):timestamp-duration*1000,
      earnedScore:record.score!=null?Number(record.score):undefined,
      totalScore:record.totalScore!=null?Number(record.totalScore):undefined,
      questionResults:(record.details||record.questionResults||[]).map((detail,i)=>{
        const q=(bank?.questions||[]).find(item=>item.id===(detail.questionId||detail.id))||null;
        const fallback=q||{id:detail.questionId||('record_q_'+idx+'_'+i),number:String(i+1),type:detail.type||'single',question:detail.question||'',options:[],answer:detail.answer||[],analysis:'',category:detail.category||''};
        return{
          question:nativeQuestionForExportV8215(fallback,i),
          userAnswer:arrayFromV8215(detail.chosen||detail.userAnswer),
          correct:!!detail.correct,
          answerText:String(detail.answerText||''),
          earnedScore:detail.score!=null?Number(detail.score):undefined,
          maxScore:detail.fullScore!=null?Number(detail.fullScore):undefined
        };
      })
    };
  });
}
function stripUndefinedV8215(value){
  if(Array.isArray(value))return value.map(stripUndefinedV8215);
  if(value&&typeof value==='object'){
    const out={};
    Object.entries(value).forEach(([k,v])=>{if(v!==undefined)out[k]=stripUndefinedV8215(v)});
    return out;
  }
  return value;
}
function buildBackupPayloadV23(banks,exportType='selected_banks',includeAll=false){
  ...
  if(includeAll){
    payload.wrongBook=nativeWrongBookForExportV8215(bankIds,true);
    payload.studyRecords=nativeStudyRecordsForExportV8215(true);
  }
  return stripUndefinedV8215(payload);
}
...
function extractBanksFromJsonImportV8215(data,fileName){
  if(Array.isArray(data)){
    if(data.some(x=>x&&typeof x==='object'&&Array.isArray(x.questions)))return data;
    return[{id:'bank_'+Date.now(),name:cleanFileNameV23(fileName).replace(/\.json$/i,'')||'导入题库',createdAt:now(),updatedAt:now(),questions:data}];
  }
  if(!data||typeof data!=='object')return[];
  if(Array.isArray(data.banks))return data.banks;
  if(Array.isArray(data.questions))return[{id:data.id||('bank_'+Date.now()),name:data.name||data.title||cleanFileNameV23(fileName).replace(/\.json$/i,'')||'导入题库',createdAt:data.createdAt||now(),updatedAt:now(),questions:data.questions}];
  if(data.bank&&Array.isArray(data.bank.questions))return[data.bank];
  if(data.questionBank&&Array.isArray(data.questionBank.questions))return[data.questionBank];
  return[];
}
function webWrongBookFromNativeV8215(rawWrongBook){
  if(!Array.isArray(rawWrongBook))return rawWrongBook&&typeof rawWrongBook==='object'?rawWrongBook:{};
  const out={};
  rawWrongBook.forEach(item=>{
    if(!item||typeof item!=='object')return;
    const bankId=String(item.bankId||'');if(!bankId)return;
    const q=item.question||{};const qid=String(q.id||item.questionId||'');if(!qid)return;
    if(!out[bankId])out[bankId]=[];
    out[bankId].push({id:qid,wrongCount:Number(item.wrongCount||1)||1,rightCount:Number(item.rightCount||0)||0,lastWrongAt:item.lastWrongAt?new Date(epochMillisV8215(item.lastWrongAt)).toISOString():'',lastCorrectAt:item.lastCorrectAt?new Date(epochMillisV8215(item.lastCorrectAt)).toISOString():'',status:item.status||'未掌握',lastAnswer:arrayFromV8215(item.lastAnswer)});
  });
  return out;
}
function webRecordsFromNativeV8215(data){
  const nativeRecords=Array.isArray(data?.studyRecords)?data.studyRecords:null;
  if(!nativeRecords)return Array.isArray(data?.records)?data.records:[];
  return nativeRecords.map((r,idx)=>({
    id:String(r.id||('rec_import_'+Date.now()+'_'+idx)),
    name:r.title||r.source||'学习记录',
    mode:r.source||'记录',
    bankId:r.bankId||'',
    bankName:r.bankName||'',
    total:Number(r.total||0)||0,
    answered:Number(r.total||0)||0,
    correct:Number(r.correct||0)||0,
    wrong:Math.max(0,(Number(r.total||0)||0)-(Number(r.correct||0)||0)),
    accuracy:r.total?Math.round((Number(r.correct||0)||0)/(Number(r.total||1)||1)*100):0,
    score:r.earnedScore??null,
    totalScore:r.totalScore??null,
    autoSubmitted:!!r.autoSubmitted,
    date:new Date(epochMillisV8215(r.timestamp)).toISOString(),
    duration:Number(r.durationSeconds||0)||0,
    details:(r.questionResults||[]).map((res,i)=>({questionId:res.question?.id||('q_'+i),question:res.question?.question||'',type:normalizeType(res.question?.type||'single')||'single',category:res.question?.category||'',chosen:arrayFromV8215(res.userAnswer),answer:arrayFromV8215(res.question?.answer),correct:!!res.correct,score:res.earnedScore??0,fullScore:res.maxScore??0,time:new Date(epochMillisV8215(r.timestamp)).toISOString()}))
  }));
}
function normalizeBackupPayloadV23(data,fileName){
  if(!data||typeof data!=='object')throw new Error('JSON 根节点不是对象');
  ...
  const banks=rawBanks.map((b,i)=>({
    ...
    updatedAt:now(),
    questions:(b.questions||[]).map((q,j)=>normalizeQuestion(q,j)).filter(q=>q.question)
  })).filter(b=>b.questions.length||b.name);
  return{
    banks,
    wrongBook:webWrongBookFromNativeV8215(data.wrongBook),
    favorites:data.favorites&&typeof data.favorites==='object'?data.favorites:{},
    records:webRecordsFromNativeV8215(data),
    settings:data.settings&&typeof data.settings==='object'?data.settings:{},
    activeBankId:data.activeBankId||''
  };
}
function jsonQuestionsForImportV8215(data){
  const banks=extractBanksFromJsonImportV8215(data,'导入题库.json');
  return banks.flatMap(b=>Array.isArray(b.questions)?b.questions:[]);
}
function parseImport(){
  if(typeof bankEditContext!=='undefined'&&bankEditContext){toast('请先保存或退出题库编辑，再开始新的识别。','warn');return}
  const textEl=$('#import-text');
  const text=textEl.value.trim();
  const strategy=$('#import-strategy')?.value||'auto';
  if(tableImportResultV49&&textEl?.dataset.tableImportV49==='1'){
  ...
  try{
    importWarnings=[];importReport='';importDiagnostics=null;
    if($('#import-mode').value==='json'||text.startsWith('[')||text.startsWith('{')){
      const data=JSON.parse(text);const arr=jsonQuestionsForImportV8215(data);
      importCache=arr.map(normalizeQuestion).filter(q=>q.question);importReport='解析策略：JSON结构化导入。';importDiagnostics={mode:'JSON结构化导入',strategy:'JSON结构化导入',profile:{},candidates:[{name:'JSON结构化导入',questions:importCache.length,score:importCache.length*10,warnings:collectImportWarnings(importCache)}],expected:{total:0,types:{}},stats:countTypes(importCache)};
    }else importCache=parseTextQuestions(text,strategy);
    importSelected.clear();
    renderImportPreview(importCache);
